...
<div class="container update-password-container">
	<div class="update-password-wrap">
		<h5 class="font-weight-bold">
			비밀번호 재설정
			<button type="button" class="btn btn-sm btn-warning py-0 float-right d-none" id="logoutBtn">logout</button>
			<button type="button" class="btn btn-sm btn-warning py-0 mx-2 float-right d-none" id="removeKeyBtn">remove key</button>
		</h5>
...
<script>
	$(function() {
		...
		$("#removeKeyBtn").on("click", function() {
			$.ajax({
				type : "DELETE",
				url : "${contextPath}/api/member/test/password-confirm/key",
				dataType : "json",
				success : function(result) {
					console.log("%c> SUCCESS", "color:green");
					console.log(JSON.stringify(result, null, 2));
